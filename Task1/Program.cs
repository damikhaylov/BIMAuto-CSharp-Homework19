            ...
            while (true) // Программа выполняется в бесконечном цикле, пока пользователь не выберет вариант выхода в меню
            {
                ...
                switch (menuPoint) // Выполнение действий согласно выбранному пункту меню
                {
                    ...
                    case 3: // Выбрать компьютеры, указав минимальный объемом ОЗУ
                        ...
                        int minRAMCapacity = -1; // Переменная для указания минимального объёма памяти
                                                 // Инициализирована по-умолчанию значением, указывающим на ошибку ввода
                        try { minRAMCapacity = Convert.ToInt32(Console.ReadLine()); }
                        catch (Exception) {}
                        Console.WriteLine();
                        if (minRAMCapacity >= 0)
                        {
